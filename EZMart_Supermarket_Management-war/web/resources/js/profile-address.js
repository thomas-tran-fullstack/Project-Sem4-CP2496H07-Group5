    ...
    try {
        ...
        if (msg) {
            ...
            if (sev === 'success') {
                ...
                // FIX: reload FULL address list, không replace từng card
                setTimeout(function () {
                    loadAddresses(); // gọi API GET /addresses
                }, 300);
            }
            else {
...
function initAddressMap() {
    // Always clean up any previous map instance on this container
    var mapContainer = document.getElementById('addr_map');
    if (mapContainer != null && mapContainer._leaflet_id !== undefined) {
        try {
            mapContainer._leaflet_id = null;
            // Remove any old map instance if possible
            if (window.addrMap && typeof window.addrMap.remove === 'function') {
                window.addrMap.remove();
            }
        } catch (e) { /* ignore */ }
    }
...
function openAddAddressModal() {
    console.log('openAddAddressModal called');
    let modal = document.getElementById('addressModal');
    console.log('modal:', modal);
    // Always move modal to body and set styles
    if (modal) {
        ...
        modal.style.zIndex = '100400';
        var inner = modal.firstElementChild;
        if (inner) inner.style.zIndex = '100500';
        console.log('modal moved to body and styles set');
    }
    modal = document.getElementById('addressModal'); // re-get in case it was moved
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Geocoding for address map images
